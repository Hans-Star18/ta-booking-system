...
export default function Index({ policies }) {
        ...
        },
        {
            ...
            cell: (row) => (
                <div className="flex items-center gap-2">
                    <Anchor
                        variant="success"
                        href={route('admin.policies.edit', row.id)}
                        className={'px-2 py-1'}
                    >
    ...
    const handleDelete = (policyId) => {
        Confirm({
            action: 'delete',
            onConfirm: () => {
                destroy(route('admin.policies.destroy', policyId), {
                    preserveScroll: true,
                    onSuccess: () => {
                        router.reload({
                            only: ['policies'],
    ...
    return (
        <>
            <Head title="Admin Policy Management" />

            <AdminLayout>
                <div className="rounded-2xl border border-gray-200 bg-white p-4 md:p-6">
                    <div className="mb-4 flex items-center justify-between">
                        <h1 className="text-2xl font-bold">Policy List</h1>
                        <Anchor
                            variant="primary"
                            href={route('admin.policies.create')}
                        >
                            Add New Policy
                        </Anchor>
                    </div>
